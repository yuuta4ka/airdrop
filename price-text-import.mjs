...
export const PRICE_JSONL_PROMPT = `Ты извлекаешь закупочные цены из PDF-прайса поставщика (файл price.pdf или приложенный PDF) и возвращаешь ТОЛЬКО валидный JSONL — по одному JSON-объекту на строку, без markdown, без пояснений, без нумерации.
...
4) sim — только для iPhone: "Sim+eSim", "eSim+eSim", "Dual Sim". Для остальных — "".
